...
13. 강의 콘텐츠의 원본은 운영 DB다. `content/demo-seed-modules.generated.json`은 `npm run seed:export`로 뽑아낸 데모용 생성물이므로 손으로 고치지 않는다. 파일로 준비한 새 강의는 `npm run module:add`로 등록하고, 기존 모듈을 파일 내용으로 덮어쓰는 `bootstrap:modules`는 새 운영 환경 최초 구축에만 쓴다.
