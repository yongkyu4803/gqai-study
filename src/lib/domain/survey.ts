...
export const toolFamiliarityOptions: {
  value: SurveyToolFamiliarity;
  label: string;
}[] = [
...
export const skillLevelOptions: { value: 1 | 2 | 3 | 4 | 5; label: string }[] =
  [
    ...
    { value: 2, label: "검색 대신 질문하는 정도" },
    {
      value: 3,
      label:
        "프롬프트를 구체적으로 써서 원하는 결과를 잘 뽑아냄 (예: 고급 구글 검색처럼 활용)",
    },
    { value: 4, label: "반복 작업을 자동화하거나 여러 단계로 연결해서 씀" },
    {
      value: 5,
      label: "API·MCP·Skill·에이전트 같은 확장 기능을 직접 써보거나 만들어봄",
    },
  ];

export const learningGoalOptions: {
  value: SurveyLearningGoal;
  label: string;
}[] = [
